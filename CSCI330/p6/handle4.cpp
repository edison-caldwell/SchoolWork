...
/*****************************************************************************/
string Creature::RaceToStr(int r)
{ 
  string str = "";
...
{
  Humanoid* h = dynamic_cast<Humanoid*>(handle);
  Monster* m = dynamic_cast<Monster*>(handle);
  
  if(h)
  {
    if(typeid(Human) == typeid(h))
      return 0;
    else if(typeid(Elf) == typeid(h))
      return 1;
    else if(typeid(HalfElf) == typeid(h))
      return 2;
    else if(typeid(Orc) == typeid(h))
      return 3;
  }
  else if(m)
  {
    if(typeid(Troll) == typeid(h))
      return 4;
    else if(typeid(Ogre) == typeid(h))
      return 5;
  }
  else
    return 0;
